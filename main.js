...
function setup() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const z = random(0, 1); // Random depth (0 = front, 1 = back)
    const size = random(20, 50); // Random size for fish
    const fish = new Fish(x, y, z, size);
    tank.addFish(fish);

    // Add the user-controlled fish
    userFish = new UserFish(500, 400, 0.5, 30); // Start in the middle of the tank
