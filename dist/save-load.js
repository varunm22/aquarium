...
const LIFE_STATES = ['normal', 'egg-laying', 'dying', 'shell', 'dead'];
// ── Module state ───────────────────────────────────────────────────────────
let modalOpen = false;
...
// ── Serialization ──────────────────────────────────────────────────────────
export function serializeTank(tank) {
    var _a;
    ...
    for (const encoded of activeCells) {
        ...
        if (wallIndex >= 0 && wallIndex < wallNames.length) {
            ...
            const level = (_a = algaeGrids[wall][x]) === null || _a === void 0 ? void 0 : _a[y];
            if (level && level > 0) {
...
export function deserializeTank(data, tank) {
    ...
    if (data.m) {
        for (const md of data.m) {
            const pos = new Position(new Vector(md.p[0], md.p[1], md.p[2]), new Vector(md.v[0], md.v[1], md.v[2]));
    ...
    if (data.g) {
        for (const ed of data.g) {
            ...
            const eggClump = new EggClump(new Vector(ed.p[0], ed.p[1], ed.p[2]), wall);
 ...
 */
function encodeLSB(imageData, data) {
...
function decodeBytesFromPixels(pixels, startBit, numBytes) {
...
function renderThumbnail(tank) {
    ...
    const tx = (x) => m + (x - TANK_CONSTANTS.X) * scaleX;
    const ty = (y) => m + (y - TANK_CONSTANTS.Y) * scaleY;
    ...
    const fishSpriteSource = EmberTetra.spritesheet
        ? EmberTetra.spritesheet.canvas || EmberTetra.spritesheet
        : null;
    if (fishSpriteSource) {
        for (const f of tank.fish) {
            if (!(f instanceof EmberTetra))
                continue;
            ...
            if (mirrored)
                ctx.scale(-1, 1);
            ctx.drawImage(fishSpriteSource, spriteConfig.x, spriteConfig.y, spriteConfig.width, spriteConfig.height, -scaledW / 2, -scaledH / 2, scaledW, scaledH);
            ctx.restore();
        }
    }
    // 7. Draw snail sprites with proper sprite selection
    const snailSpriteSource = Snail.spritesheet
        ? Snail.spritesheet.canvas || Snail.spritesheet
        : null;
    if (snailSpriteSource) {
        for (const snail of tank.getSnails()) {
            if (snail.getLifeState() === 'dead')
                continue;
            ...
            if (finalMirrored)
                ctx.scale(-1, 1);
            ctx.drawImage(snailSpriteSource, config.x, config.y, config.width, config.height, -scaledW / 2, -scaledH / 2, scaledW, scaledH);
            ctx.restore();
        }
    }
    // 8. Water line (front)
...
// ── Save flow ──────────────────────────────────────────────────────────────
export function handleSave(tank) {
    return __awaiter(this, void 0, void 0, function* () {
        if (modalOpen)
            return;
        modalOpen = true;
        try {
            ...
            // 2. Compress
            const compressed = yield compress(jsonBytes);
...
function showSaveModal(dataUrl, blob, jsonSize, compressedSize) {
...
// ── Load flow ──────────────────────────────────────────────────────────────
export function handleLoad(tank) {
    return __awaiter(this, void 0, void 0, function* () {
        if (modalOpen)
            return;
        modalOpen = true;
...
function showLoadModal(tank) {
    ...
    function processFile(file) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                // Decompress
                const decompressed = yield decompress(rawData);
                const json = new TextDecoder().decode(decompressed);
                loadedData = JSON.parse(json);
                // Count entities
                const fishCount = ((_a = loadedData.f) === null || _a === void 0 ? void 0 : _a.length) || 0;
                const snailCount = ((_b = loadedData.n) === null || _b === void 0 ? void 0 : _b.length) || 0;
                const microCount = ((_c = loadedData.m) === null || _c === void 0 ? void 0 : _c.length) || 0;
                const foodCount = ((_d = loadedData.d) === null || _d === void 0 ? void 0 : _d.length) || 0;
                const eggCount = ((_e = loadedData.g) === null || _e === void 0 ? void 0 : _e.length) || 0;
                const algaeCount = ((_f = loadedData.a) === null || _f === void 0 ? void 0 : _f.length) || 0;
    ...
    loadBtn.addEventListener('click', () => {
        ...
        try {
            ...
            errorMsg.textContent = 'Failed to load state: ' + err.message;
            errorMsg.style.display = 'block';
        }
    });
