...
export class Algae {
    ...
    constructor() {
        ...
        this.spreadChance = 0.003; // 1% chance per frame for spreading
    ...
    getAlgaeLevel(wall, worldX, worldY, worldZ) {
    ...
    removeAlgae(wall, worldX, worldY, worldZ) {
        // Convert world coordinates to grid coordinates
        const gridCoords = this.worldToGridCoordinates(wall, worldX, worldY, worldZ);
        if (!gridCoords)
            return;
    ...
    findBestAlgaeTarget(snailX, snailY, snailZ, maxDistance = 600) {
        ...
        for (const hotspot of this.algaeHotspots) {
            const distance = Math.sqrt(Math.pow(hotspot.centerX - snailX, 2) +
