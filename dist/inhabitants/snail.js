...
export class Snail extends Inhabitant {
    constructor(size = 20, initialPosition, initialWall, initialHunger) {
        ...
        this.coveredAlgaePositions = new Set();
        this.tank = null;
        this.bounds = getTankBounds();
        this.eatingCounter = 0;
        this.goalReEvaluationChance = 0.003;
        this.maxSize = 50;
        this.frameCounter = 0;
        this.ALGAE_INTERACTION_INTERVAL = 5;
        this.lastTransitionFrame = 0;
        this.TRANSITION_COOLDOWN = 10;
        this.lastGoalSetFrame = 0;
    ...
    getRandomPositionOnWall(wall, offset) {
        const bounds = this.bounds;
        const pos = new Vector(bounds.min.x + random() * (bounds.max.x - bounds.min.x), bounds.min.y + random() * (bounds.max.y - bounds.min.y), bounds.min.z + random() * (bounds.max.z - bounds.min.z));
    ...
    findBestAlgaeTarget() {
        ...
        for (const hotspot of this.tank.algae.getAlgaeHotspots()) {
            ...
            if (distance > HOTSPOT_MAX_DISTANCE)
                continue;
    ...
    findPreciseAlgaeGoal(hotspotGoal) {
        ...
        for (let squareX = regionMinX; squareX < regionMaxX; squareX += PRECISE_SQUARE_SIZE) {
            for (let squareY = regionMinY; squareY < regionMaxY; squareY += PRECISE_SQUARE_SIZE) {
                const squareStrength = this.calculateSquareAlgaeStrength(hotspotGoal.wall, squareX, squareY, PRECISE_SQUARE_SIZE);
    ...
    checkWallTransition() {
        ...
        if (targetWall === this.wall)
            return false;
        if (!getAdjacentWalls(this.wall).includes(targetWall))
            return false;
        const bounds = this.bounds;
        const pos = this.position.value;
