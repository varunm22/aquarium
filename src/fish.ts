...
export class Fish extends Inhabitant {
    private static spritesheet: p5.Image | null = null;
    private static readonly SPRITE_CONFIGS: SpriteConfig[] = [
    ...
    private getSpriteIndex(): { index: number; mirrored: boolean } {
      ...
      // Map to octants
      if (degrees >= 337.5 || degrees < 22.5) return { index: 0, mirrored: false }; // front
      if (degrees >= 22.5 && degrees < 67.5) return { index: 1, mirrored: false }; // front-left
      if (degrees >= 67.5 && degrees < 112.5) return { index: 2, mirrored: false }; // left
      if (degrees >= 112.5 && degrees < 157.5) return { index: 3, mirrored: false }; // left-back
      if (degrees >= 157.5 && degrees < 202.5) return { index: 4, mirrored: false }; // back
      if (degrees >= 202.5 && degrees < 247.5) return { index: 3, mirrored: true }; // back-right
      if (degrees >= 247.5 && degrees < 292.5) return { index: 2, mirrored: true }; // right
